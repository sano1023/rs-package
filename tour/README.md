...
npm install https://github.com/sano1023/ryusuke-packages-dist/raw/main/tarballs/tour-0.1.0.tgz
```

```html
<!-- CDN（jsDelivr・scriptタグ直読み） -->
<script src="https://cdn.jsdelivr.net/gh/sano1023/ryusuke-packages-dist@main/tour/dist/tour.min.js"></script>
```

```js
// ESM import（npmインストール後）
import { /* 公開API */ } from '@knoweble/spotlight-tour';
```

CSSが必要なパッケージは `dist/tour.css` を link してください。

---

# @knoweble/spotlight-tour
...
```js
import { createTour } from './spotlight-tour/index.js';
// npm公開後: import { createTour } from '@knoweble/spotlight-tour';
```
...
```js
import { createTour } from '@knoweble/spotlight-tour';
...
function TourButton() {
    const tour = useMemo(() => createTour({ steps: [/* ... */] }), []);
